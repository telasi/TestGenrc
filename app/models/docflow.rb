...
class Docflow < ActiveRecord::Base
  ...
  self.table_name = 'docflow'

  def self.start(params)
  	params.merge!({stage: 1, stage_1_date: Time.now})
  	docflow = Docflow.create!(params)
  ...
  def send_to_gnerc
 	  Sender.perform_async('Docflow', self.id)
  end

  def put_to_queue
    SendQueue.create!(service: 'Docflow', service_id: self.id, stage: self.stage)
  end
